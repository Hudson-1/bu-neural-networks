...
df = pd.read_csv('sine_wave_compare.csv')
...
pd.DataFrame(IND_DATA.reshape(9995, -1)).to_csv('ind_data_compare.csv', index=False, header=ind_cols)

# Save DEP_DATA as a csv file named dep_data.csv with shape (9995, 3) and column names
pd.DataFrame(DEP_DATA).to_csv('dep_data_compare.csv', index=False, header=dep_cols)
