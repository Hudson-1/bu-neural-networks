...
for i in range(0,n_data - win_len, win_len):
...
print("ind ",IND_DATA.shape)
# Save IND_DATA as a csv file named ind_data_compare.csv with shape (9969, 30, 3) and column names
pd.DataFrame(IND_DATA.reshape(IND_DATA.shape[0], -1)).to_csv('CNN\CNN_Data\ind_data_cnn.csv', index=False, header=ind_cols)
